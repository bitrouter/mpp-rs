...
use mpp_br::client::{Fetch, PaymentProvider, TempoProvider};
...
async fn test_zero_amount_identity_flow_uses_proof_credential() {
    ...
    let auth_header = mpp_br::format_authorization(&credential).expect("failed to format credential");
...
async fn test_keychain_source_did_consistent_across_proof_and_paid() {
    use mpp_br::client::tempo::signing::{KeychainVersion, TempoSigningMode};
    ...
    let identity_challenge =
        mpp_br::parse_www_authenticate(www_auth).expect("failed to parse identity challenge");
    ...
    let www_auth = paid_resp
        ...
        .unwrap();
    let paid_challenge =
        mpp_br::parse_www_authenticate(www_auth).expect("failed to parse paid challenge");
